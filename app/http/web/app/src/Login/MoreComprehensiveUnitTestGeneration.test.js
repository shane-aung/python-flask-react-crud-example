...
describe('Login', () => {
    ...
    it('handles errors from signInWithRedirect', async () => {
      mockOktaAuth.signInWithRedirect.mockRejectedValue(new Error('Login failed'));
      render(<Login oktaAuth={mockOktaAuth} authState={{ isAuthenticated: false }} />);
  
      const loginButton = screen.getByRole('button', { name: 'Login with Okta' });
      await userEvent.click(loginButton);
  
      // Check for error handling or UI updates (e.g., error message)
    });
  });
